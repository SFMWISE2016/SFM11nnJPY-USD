...
# See also:    SFEnnarch
# ---------------------------------------------------------------------
# Quantlet:    SFEnnjpyusd
# ---------------------------------------------------------------------
# Description: plots result of a radial basis function (rbf) neural network
#              for exchange rate JPY/USD data set
#              (80% of data as training set and 20% as validation set)
# ---------------------------------------------------------------------
# Usage:       -
# ---------------------------------------------------------------------
# Keywords:    Neural Network
# ---------------------------------------------------------------------
# Author:      Awdesch Melzer 20130801
# ---------------------------------------------------------------------
...
# Output      
# Parameter   inp
# Definition  list of input parameters which define the network# it consists of elements
...
# Parameter   net
# Definition  list of network characteristics containing: 
#
...
# Parameter   err
# Definition  list of output error functions containing:
#
...
#  warning((inputs > 24)||(outputs > 10),"rbftrain: the dimensions of the data are too high")
...
# normalize
  ...
  x = (x - matrix(minx,NROW(x),NCOL(x),byrow=T)) / (matrix(maxx,NROW(x),NCOL(x),byrow=T) - matrix(minx,NROW(x),NCOL(x),byrow=T))
  y = (y - matrix(miny,NROW(y),NCOL(y),byrow=T)) / (matrix(maxy,NROW(y),NCOL(y),byrow=T) - matrix(miny,NROW(y),NCOL(y),byrow=T))
...
# find the nearest cluster
      ...
      clustersOutput = sqrt( apply((clustersWeights - X)*(clustersWeights - X),1,sum) )	# eucl. distance: i-th point from cluster centers
...
# update weights of the nearest cluster
    ...
    interimLR = (learn[2] - (learn[2]-learn[1])/max(c(1,(epochs[1]-1)))*(e1-1))  *  interimLR	#//!!! was above "clustersWeights[clusterChamp,] = ..."
...
# give some info
  ...
  inp=list(inputs=inputs,clusters=clusters,outputs=outputs,samples=samples,learn=learn,epochs=epochs,mMSE=mMSE,activ=activ)  # 1|1|1|1|3|2|1|1
  net=list(clustersWeights=clustersWeights,trFonDev=trFonDev,outputsWeights=outputsWeights,bias=bias) # (clusters,inputs), (clusters), (outputs,clusters), (outputs)
...
# Definition  (n x p) matrix, predictor variables
# Parameter   rbfnet
# Definition  composed object (list), RBF network as computed by rbftrain or rbftrain2
# Parameter   miny
# Definition  (q x 1) vector, minimum values for the output vector
# Parameter   maxy
# Definition  (q x 1) vector, maximum values for the output vector
# ---------------------------------------------------------------------
# Output      
# Parameter   netOut
# Definition  (n x q) matrix, output of the rbfnet
# ---------------------------------------------------------------------
# set parameters
  if(sum(miny >= maxy)>0){
...
################################ MAIN PROGRAM ################################
  ...
  r  = nrow(x)                      
  xt = x[4:(4*nrow(x)/5),2]   # 80% of data as training set,
...
# selects number of clusters, learning rate,
...
# plots real data (red)  versus result from network (blue)
  time  = (1:nrow(train.tt))/257 + (1994)
  tr = cbind(time,rbf) # blue thin line
  ytr= cbind(time,train.tt) # red thin line  

 
# OUT OF SAMPLE:  
# 20% of data, prepares matrix
  
  xt=x[(4*NROW(x)/5+1):NROW(x),2]
  xt = as.matrix(as.numeric(as.character(xt)))
  #==================lagging
ncol(xt)
nrow(xt)                
